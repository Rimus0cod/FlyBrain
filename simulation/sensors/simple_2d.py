...
class Simple2DSensors:
    """Four panoramic beacon receptors plus a normalized angular-rate sensor.
    ...
    vision_dim = 4
    inertial_dim = 1
    observation_dim = vision_dim + inertial_dim
    ...
    def reset(self) -> None:
        """Reset temporal sensor state (reserved for future motion features)."""
    ...
    def observe(self, body_state: dict[str, torch.Tensor], target: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat((visual.clamp(0.0, 1.0), angular_velocity.clamp(-1.0, 1.0)), dim=-1)
